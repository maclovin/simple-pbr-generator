...
def compute_height_map(input_file):
    img = Image.open(input_file)
    img = img.convert("L")

    return img

def compute_ao_map(input_file):
    img = Image.open(input_file)
    img = img.convert("L")

    enhacer_brightness = ImageEnhance.Brightness(img)
    img = enhacer_brightness.enhance(5)

    enhacer_contrast = ImageEnhance.Contrast(img)
    img = enhacer_contrast.enhance(.1)
...
def main():
    ...
    parser.add_argument('-o','--output_file', default=None, type=str, help='output image path')
    ...
    height_map = compute_height_map(input_file)
    ao_map = compute_ao_map(input_file)

    normal_file_to_save = output_file if not output_file == None else input_file[0:-4] + "_normal.png"
    height_file_to_save = output_file if not output_file == None else input_file[0:-4] + "_height.png"
    ao_file_to_save = output_file if not output_file == None else input_file[0:-4] + "_ao.png"
        
    imageio.imwrite(normal_file_to_save, normal_map)
    height_map.save(height_file_to_save)
    ao_map.save(ao_file_to_save)
